...
@OnlyIn(Dist.CLIENT)
public class ModelFairling extends HierarchicalModel<Fairling> implements ArmedModel {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(Otherworld.MODID, "fairling"), "main");
    ...
    public ModelFairling(ModelPart root) {
    ...
    public void setupAnim(Fairling fairling, float p_233326_, float p_233327_, float p_233328_, float p_233329_, float p_233330_) {
        this.root().getAllParts().forEach(ModelPart::resetPose);
        float f2 = p_233328_ - (float)fairling.tickCount;
        float f3 = p_233328_ * 9.0F * ((float)Math.PI / 180F);
        float f4 = Math.min(p_233327_ / 0.3F, 1.0F);
        float f5 = 1.0F - f4;
        float f6 = fairling.getHoldingItemAnimationProgress(f2);
        if (fairling.isHappy()) {
            float f7 = p_233328_ * 8.0F * ((float)Math.PI / 180F) + p_233327_;
            float f8 = Mth.cos(f7) * 16.0F * ((float)Math.PI / 180F);
            float f9 = fairling.getSpinningProgress(f2);
            float f10 = Mth.cos(f7) * 14.0F * ((float)Math.PI / 180F);
            float f11 = Mth.cos(f7) * 30.0F * ((float)Math.PI / 180F);
            this.root.yRot = fairling.isSpinning() ? 12.566371F * f9 : this.root.yRot;
        ...
        this.left_wing.yRot = (1.0995574F + Mth.cos(p_233328_ * 120.32113F * ((float)Math.PI / 180F)) * ((float)Math.PI / 180F) * 32.2F) / 2.5F;
